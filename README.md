...
- **AI**: Claude / OpenAI (ChatGPT) / Gemini（链接与图片解析，可切换）
- **Deploy**: Vercel
...
| `CLAUDE_API_KEY` | Claude API 密钥（PARSE_MODE=claude） |
| `OPENAI_API_KEY` | OpenAI API 密钥（PARSE_MODE=openai，ChatGPT） |
| `GEMINI_API_KEY` | Google Gemini API 密钥（PARSE_MODE=gemini） |
| `PARSE_MODE` | `mock` / `claude` / `openai` / `gemini` |
