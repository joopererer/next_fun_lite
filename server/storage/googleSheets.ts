...
export class GoogleSheetsAdapter implements StorageAdapter {
  ...
  async getProfile(_userId: string): Promise<import('../../shared/types').Profile | null> {
    return null
  }

  async upsertProfile(data: Partial<import('../../shared/types').Profile> & { id: string; nickname?: string }): Promise<import('../../shared/types').Profile> {
    throw new Error('Profiles not supported in Google Sheets backend')
  }
